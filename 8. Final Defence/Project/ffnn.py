...
def init_parameters(layers_dim, initialization="random"):
    ...
    for l in range(1, L):
        ...
        if initialization == "he":
            # he-initialization
            params['W' + str(l)] = np.random.randn(layers_dim[l], layers_dim[l - 1]) * np.sqrt(
        ...
        layers_dim[l], layers_dim[l - 1])), "Dimention of W mismatched in init_params function"
...
def backward_prop(AL, Y, caches, dropout_masks=[], keep_probs=[], lambd=0, regularizer=None):
    ...
    grads["dA" + str(L - 1)], grads["dW" + str(L)], grads["db" + str(L)] = backward_activation(dA, current_cache,
                                                                                               lambd=lambd,
    ...
    for l in reversed(range(L - 1)):
        ...
        if regularizer == "dropout":
            ...
            dA_prev, dW_temp, db_temp = backward_activation(dA_prev_temp, current_cache, lambd=lambd,
                                                            regularizer=regularizer, activation='relu')
        else:
            dA_prev, dW_temp, db_temp = backward_activation(grads["dA" + str(l + 1)], current_cache, lambd=lambd,
                                                            regularizer=regularizer, activation='relu')
    ...
    for l in range(L):
        if optimizer == 'adam':
            ...
            parameters["b" + str(l + 1)] = np.subtract(parameters["b" + str(l + 1)],
                                                       learning_rate * np.divide(v_corrected["db" + str(l + 1)],
